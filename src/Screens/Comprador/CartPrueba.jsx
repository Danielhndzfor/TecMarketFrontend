...
import { getCart, addToCart, removeFromCart, clearCart, increaseQuantity, decreaseQuantity } from '../../api/cart';
...
const Cart = () => {
    ...
    const [cartId, setCartId] = useState(null); // Estado para cartId
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [products, setProducts] = useState({}); // Estructura para productos
    ...
    const handleQuantityChange = (productId, newQuantity) => {
        ...
        if (newQuantity < 1) {
            const confirmDelete = window.confirm('¿Estás seguro de que deseas eliminar este producto del carrito?');
            if (confirmDelete) {
                handleRemove(productId); // Eliminar el producto
            } else {
                // Si no se confirma, restablecer la cantidad a 1
                newQuantity = 1;
            }
        }
        ...
        const newTimer = setTimeout(async () => {
            try {
                // Solo se hace un POST para actualizar la cantidad
                await updateQuantityInCart(userId, productId, newQuantity);
            } catch (error) {
    ...
    // Función que realiza el POST para actualizar la cantidad del producto
    const updateQuantityInCart = async (userId, productId, newQuantity) => {
        try {
            // Llamada a la API para actualizar la cantidad del producto
